...
ParseMst::ParseMst(ObjectRegistry *objects, const char *id, int threadIndex): Module(objects, id, threadIndex) {
	items = 0;
	parserDir = NULL;
	model = NULL;
	...
	props = new ObjectProperties<ParseMst>(this);
	props->Add("items", &ParseMst::GetItems);
	props->Add("parserDir", &ParseMst::GetParserDir, &ParseMst::SetParserDir, true);
	props->Add("model", &ParseMst::GetModel, &ParseMst::SetModel, true);
	props->Add("decodeType", &ParseMst::GetDecodeType, &ParseMst::SetDecodeType, true);
	props->Add("order", &ParseMst::GetOrder, &ParseMst::SetOrder, true);
...
char *ParseMst::GetItems(const char *name) {
	return int2str(items);
}

char *ParseMst::GetParserDir(const char *name) {
	return strdup(parserDir);
}

void ParseMst::SetParserDir(const char *name, const char *value) {
	free(parserDir);
	parserDir = strdup(value);
}

char *ParseMst::GetModel(const char *name) {
	return strdup(model);
}

void ParseMst::SetModel(const char *name, const char *value) {
	free(model);
	model = strdup(value);
}

char *ParseMst::GetDecodeType(const char *name) {
	return strdup(decodeType);
}

void ParseMst::SetDecodeType(const char *name, const char *value) {
	free(decodeType);
	decodeType = strdup(value);
}

char *ParseMst::GetOrder(const char *name) {
	return int2str(order);
}

void ParseMst::SetOrder(const char *name, const char *value) {
	order = str2int(value);
}

bool ParseMst::Init(vector<pair<string, string> > *params) {
	// second stage?
	if (!params)
		return true;
...
bool ParseMst::ReadWrite(string &writeBuffer, string &readBuffer, bool waitForRead) {
...
string &ParseMst::GetShortTag(string &tag) {
...
bool ParseMst::ParseSentence(vector<string> &form, vector<string> &tag, vector<int> &head, vector<string> &depRel) {
	head.clear();
	depRel.clear();
	string a;
	string b;
...
Resource *ParseMst::ProcessSimpleSync(Resource *resource) {
...
extern "C" Module* hector_module_create(ObjectRegistry *objects, const char *id, int threadIndex) {
	return new ParseMst(objects, id, threadIndex);
}
