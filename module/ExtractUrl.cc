...
ExtractUrl::ExtractUrl(ObjectRegistry *objects, const char *id, int threadIndex): Module(objects, id, threadIndex) {
	items = 0;
	newUrlStatus = 0;
	imageLinks = false;
	allowedSchemes = "http";
	...
	props = new ObjectProperties<ExtractUrl>(this);
	props->Add("items", &ExtractUrl::GetItems);
	props->Add("newUrlStatus", &ExtractUrl::GetNewUrlStatus, &ExtractUrl::SetNewUrlStatus);
	props->Add("imageLinks", &ExtractUrl::GetImageLinks, &ExtractUrl::SetImageLinks);
	props->Add("allowedSchemes", &ExtractUrl::GetAllowedSchemes, &ExtractUrl::SetAllowedSchemes);
...
char *ExtractUrl::GetItems(const char *name) {
	return int2str(items);
}

char *ExtractUrl::GetNewUrlStatus(const char *name) {
	return int2str(newUrlStatus);
}

void ExtractUrl::SetNewUrlStatus(const char *name, const char *value) {
	newUrlStatus = str2int(value);
}

char *ExtractUrl::GetImageLinks(const char *name) {
	return bool2str(imageLinks);
}

void ExtractUrl::SetImageLinks(const char *name, const char *value) {
	imageLinks = str2bool(value);
}

char *ExtractUrl::GetAllowedSchemes(const char *name) {
	return strdup(allowedSchemes.c_str());
}

void ExtractUrl::SetAllowedSchemes(const char *name, const char *value) {
...
bool ExtractUrl::Init(vector<pair<string, string> > *params) {
	// second stage?
	if (!params)
		return true;
...
void ExtractUrl::AppendUrl(const char *url, GURL *base, queue<Resource*> *outputResources) {
	bool do_free = false;
...
bool ExtractUrl::ProcessMultiSync(queue<Resource*> *inputResources, queue<Resource*> *outputResources, int *expectingResources, int *processingResources) {
...
extern "C" Module* hector_module_create(ObjectRegistry *objects, const char *id, int threadIndex) {
	return new ExtractUrl(objects, id, threadIndex);
}
