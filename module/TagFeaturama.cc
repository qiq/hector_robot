...
#include "TagFeaturama.h"
#include "TextResource.h"

using namespace std;

TagFeaturama::TagFeaturama(ObjectRegistry *objects, const char *id, int threadIndex): Module(objects, id, threadIndex) {
	items = 0;
	morphologyPrefix = NULL;
	useGuesser = true;
	taggerPrefix = NULL;

	props = new ObjectProperties<TagFeaturama>(this);
	props->Add("items", &TagFeaturama::GetItems);
	props->Add("morphologyPrefix", &TagFeaturama::GetMorphologyPrefix, &TagFeaturama::SetMorphologyPrefix, true);
	props->Add("useGuesser", &TagFeaturama::GetUseGuesser, &TagFeaturama::SetUseGuesser, true);
	props->Add("taggerPrefix", &TagFeaturama::GetTaggerPrefix, &TagFeaturama::SetTaggerPrefix, true);
...
char *TagFeaturama::GetItems(const char *name) {
	return int2str(items);
}

char *TagFeaturama::GetMorphologyPrefix(const char *name) {
	return strdup(morphologyPrefix);
}

void TagFeaturama::SetMorphologyPrefix(const char *name, const char *value) {
	free(morphologyPrefix);
	morphologyPrefix = strdup(value);
}

char *TagFeaturama::GetUseGuesser(const char *name) {
	return bool2str(useGuesser);
}

void TagFeaturama::SetUseGuesser(const char *name, const char *value) {
	useGuesser = str2bool(value);
}

char *TagFeaturama::GetTaggerPrefix(const char *name) {
	return strdup(taggerPrefix);
}

void TagFeaturama::SetTaggerPrefix(const char *name, const char *value) {
	free(taggerPrefix);
	taggerPrefix = strdup(value);
}

bool TagFeaturama::Init(vector<pair<string, string> > *params) {
	int result;
...
string TagFeaturama::CstsEncode(const char *src) {
	string result;
	while (*src) {
...
string TagFeaturama::CstsEncodeLemma(const char *src, int len) {
	string result;
	while (len) {
		if (*src == '-' || *src == '_' || *src == '`') {
...
string TagFeaturama::MorphologyToOffer(const char *s) {
	string result;
	string lemma;
	const char *l = s;
	const char *t = NULL;
...
Resource *TagFeaturama::ProcessSimpleSync(Resource *resource) {
...
extern "C" Module* hector_module_create(ObjectRegistry *objects, const char *id, int threadIndex) {
	return new TagFeaturama(objects, id, threadIndex);
}
