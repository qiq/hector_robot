...
for l in cs de en sk; do
	hector_server -c Language.xml -fb test_gcld data/${l}_test.hr.gz 2>&1 | cat >>evaluate_gcld.txt
	hector_server -c Language.xml -fb test_top_words data/${l}_test.hr.gz 2>&1 | cat >>evaluate_top_words.txt
done
rm -f /tmp/delme.$$
